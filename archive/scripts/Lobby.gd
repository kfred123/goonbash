...
const API_URL = "https://localhost:5281/api/lobbies"
...
func _on_http_request_completed(result, response_code, headers, body):
	if response_code == 200:
		...
		connect_status.text = "Failed to fetch lobbies. %s - %s" % [response_code, body.get_string_from_utf8()]
	...
	if mode == "local":
		...
		create_req.request_completed.connect(func(res, code, headers, body):
			...
			if code == 200:
				...
				connect_status.text = "Failed to create online game. %s" % [code]
		)
