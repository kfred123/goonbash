...
- **Name storage (client)**: Keep the player name in a small controller-owned value backed by `localStorage`, not a new global framework. Rationale: the project has no state-management library; a minimal local field is consistent with the existing scene's style and avoids a new dependency. `localStorage` (rather than a cookie or `sessionStorage`) is used because it persists across browser restarts without being sent on every HTTP request, requires no server-side cookie handling, and needs no consent banner since no tracking/identification data leaves the browser.
...
- None outstanding; defaults above (20-char cap, `localStorage`, fallback name `Player`) are treated as accepted decisions for this change.
