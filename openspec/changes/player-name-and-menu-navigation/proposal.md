...
- Persist the entered name for the session and require a non-empty name before the player can proceed to the lobby menu.
...
- `lobby-team-management`: Requires the player's name to be included in the synchronized lobby roster, and clarifies that a lobby SHALL be removed once it has no remaining active players, covering both waiting and started lobbies.
...
- `backend/src/rooms/GameRoom.ts`: accept and store the player name on join, keep existing lobby-cleanup-on-leave behavior working for both waiting and started lobbies.
